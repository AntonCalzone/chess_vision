    ...
    def find_squares(self, image: np.ndarray) -> list:
        ...
        for i, contour in enumerate(board_contours):
            if self.min_contour_area < cv2.contourArea(contour) < self.max_contour_area:
                ...
                center_x = (x + (x + w)) / 2
                center_y = (y + (y + h)) / 2
                square_centers.append([center_x, center_y, pt2, pt1, pt3, pt4, area])
        ...
        # Calculate horizontal line passing through each square in every row
        row_lines = []
